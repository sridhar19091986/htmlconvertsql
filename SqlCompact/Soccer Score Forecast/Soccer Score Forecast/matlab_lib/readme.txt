 	...
 	B. Set the path by doing one of the following:
		...
		* Add the MCR directory to the environment variable by issuing 
         ...
         NOTE: When deploying C and C++ standalone applications, it is possible
               to run the shell script file run_CurveFit.sh on UNIX and Mac
...
3. Collect the following files, based on what you are deploying: 
	...
	All Platforms:
		* CurveFit.ctf (Component Technology File archive,
		  platform-dependent on target machine)
			
        Windows:
		* MCRInstaller.exe (self-extracting MATLAB Component Runtime
		  library installer)   
		* CurveFit.exe (application created by MATLAB Compiler)
        UNIX:
		* MCRInstaller.bin (MATLAB Component Runtime library 
	 	  installer)
		* CurveFit (application created by MATLAB Compiler)
		* run_CurveFit.sh (shell script that can be run 
	...
	All Platforms:            
		* CurveFit.dll (shared library, file extension 
		  varies by platform)
		* CurveFit.h (library header file)
		* CurveFit.ctf (Component Technology File archive,
	...
	* CurveFit.xml (documentation files)
	* CurveFit.pdb (program database file - if 
	  DEBUG option selected)
	* CurveFit.dll (component assembly file)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  
	...
	* CurveFit.ctf (Component Technology File archive, 
	  contains compiled M-code)
	* CurveFit_0_0.dll (COM component)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  	
	...
	* CurveFit.jar (Java class files and compiled M-code)
	...
	* CurveFit.ctf (Component Technology File archive, 
	  contains compiled M-code)
	* CurveFit.xla (Optionally created and placed in
	  D:\My Documents\MATLAB\CurveFit\src directory by the user)  
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  	
