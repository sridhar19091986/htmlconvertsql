...
namespace Soccer_Score_Forecast
{
    public partial class LoadDataToTree : LoadDataMethod
    {
        ...
        private void LoadTypeTree(TreeView tv)
        {
            ...
            tv.Nodes.Add(root);
            //选定match_type过滤
            var mt = ltlAll.Values.Select(e => e.Match_type).Distinct();
            ...
            foreach (var m in mt)
            {
                ...
                root.Nodes.Add(tn);
                ltls = ltlAll.Values.Where(p => p.Match_type == m);
                TreeNodeLoad(tn);
            }
        }
        private void LoadTimeTree(TreeView tv)
        {
            ...
            tv.Nodes.Add(root);
            //选定match_type过滤
            var mt = ltlAll.Values.Select(e => e.Match_time).Distinct();
            ...
            foreach (var m in mt)
            {
                ...
                root.Nodes.Add(tn);
                ltls = ltlAll.Values.Where(p => p.Match_time == m);
                TreeNodeLoad(tn);
            }
        }
