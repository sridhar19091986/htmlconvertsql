...
namespace Soccer_Score_Forecast
{
    public class UpdateAnalysisResult
    {
        ...
        {
            mar = matches.Match_analysis_result.Where(e => e.Result_tb_lib_id == null || e.Result_fit == "no forecast");
            ...
            if (overday != -1)
                ...
                mar = matches.Match_analysis_result.OrderByDescending(e => e.Result_tb_lib_id).Take(2000);
             * */
        ...
        public void ExecUpdate()
        {
            ...
            foreach (var m in mar)
            {
                ...
                if (rtls.Contains(comparekeyA) || rtls.Contains(comparekeyB) || rtls.Contains(comparekeyC))
                {
                    ...
                    //2011.11.2 更正为grnn预测校正。

                    m.Result_fit = CompareGrnnResult(m.Grnn_fit, rtl.Full_home_goals, rtl.Full_away_goals);
        ...
        private string CompareGrnnResult(string grnn, int? homegoals, int? awaygoals)
        {
            string result = "no forecast";
            ...
            if (goals_grnn[2] - goals_grnn[3] == homegoals - awaygoals
                || (goals_grnn[2] - goals_grnn[3]) * (homegoals - awaygoals) > 0) result = "yes";
            else result = "no";
            return result;
        }
    }
